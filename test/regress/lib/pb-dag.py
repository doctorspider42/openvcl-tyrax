#!/usr/bin/env python3
"""The value-DAG oracle, extended across control flow.
...
def run_trace(prog, sem, has_delay, decide, unroll=UNROLL):
    ...
    return m.obs, tuple(keys), decisions
...
def judge(sprog, eprog, name):
    ...
    for pname, pol in policies(nbranch):
        try:
            want, kw, decisions = run_trace(sprog, True, False, pol)
        ...
        seen.add(key)
        try:
            got, kg, _ = run_trace(eprog, False, True, replay(decisions))
        except (Unsupported, Budget) as e:
        ...
        for kind, text, sample in compare(want, got):
            ...
            break
        if problems and problems[-1][0] not in ("  source-only", "  emitted-only"):
            break
        if len(problems) >= 3:
            break
    return n, problems
    ...
    for d in sys.argv[3:]:
        ...
        for f in sorted(os.listdir(d)):
            ...
            try:
                ...
                skipped += 1
                continue
            n, problems = judge(sprog, eprog, f[:-4])
            dtr += n
            traces += n
        ...
        print("  %d programs in scope, %d divergent, %d traces" % (dtot, dbad, dtr))
